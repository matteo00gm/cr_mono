...
- **The wrapper has to be a second declaration of the extension's C entry point**, which is PostgreSQL's own recipe from the `unaccent` documentation — and finding out why took two CI runs. A SQL wrapper marked `IMMUTABLE` does not work: without a `SET` clause it is *inlinable*, so Postgres expands it and sees the `STABLE` `unaccent` underneath (`42P17`); with a `SET search_path` clause it is not inlinable and still fails, because a function carrying one cannot appear in a stored generation expression either. Declaring the C function directly leaves nothing mutable for inlining to expose.
- **The immutability is asserted rather than true, and that is the trade.** It asserts immutability the underlying function does not have. What that costs: editing `unaccent.rules` on a running database leaves existing rows indexed under the old folding until they are rewritten. What it buys: `nebbiolo` matching `Nebbiòlo`, which is what Italian visitors actually type — and the only alternative is unaccenting at query time, which cannot use an index at all. Every name inside the function is schema-qualified, which is what stops a caller redirecting resolution with their own `search_path` — the escalation shape a function marked `IMMUTABLE` and called during an index build would otherwise have.
