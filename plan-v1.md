...
**A third, found by CI after the first push, and worth recording because it is a repeat.** `pgErrorCode` read the SQLSTATE from `error.cause.code` only — which is where `db.execute` leaves it, and *not* where the query builder does: postgres-js's own `PostgresError` carries `code` on the error itself, and Drizzle's builder passes it through untouched. So a duplicate SKU escaped as a 500 while every unit test stayed green, because the fake had been written from the implementation's assumption instead of from the driver's behaviour. **That is A1's timestamp bug again, from the same cause.** The function now reads both levels and the unit test builds both shapes, naming which call site produces each.
