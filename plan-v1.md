...
**As built — the component named in the row could not be used, and the reason is P0-17a.** `sst.aws.StaticSite` brings its own bucket *and* its own CloudFront distribution. This application already has one built by hand, carrying a per-behaviour SPA-rewrite function, an API origin with the A2 secret, and the ordering warning P3 depends on. Adopting the component would mean either a second distribution on a second domain — which breaks the same-origin session cookie the whole auth design rests on (D5) — or unpicking P0-17a. So the build is uploaded as objects into the bucket that already exists, and what the component would have decided is decided in `infra/static-assets.ts` instead.

Four things came out of writing it:

- **The cache rule is opt-in, not opt-out.** `immutable` applies only to names carrying a Vite content hash under `assets/`; everything else revalidates, including a file somebody drops into `assets/` by hand. The consequences are asymmetric and that is what decides the default: a file wrongly frozen is cached for a year in browsers nobody can reach, because there is no invalidation for a client-side cache — only a new URL — while a file wrongly revalidating costs one conditional request. A service worker is the case where getting this backwards is unrecoverable without a change users cannot receive.
- **`infra/` gained its first test, and it is the file that carries a decision.** Everything else in that directory is resource declarations whose behaviour is AWS's; `static-assets.ts` chooses which files may be cached for a year, and getting it wrong ships silently. `typecheck:infra` is local-only (**E3**), so `infra/` is otherwise guarded only by CI greps — which can check that a string appears, not that it appears against the right file. `vitest.config.ts` gains an `infra` project for it. The rule was verified by inverting it and watching five tests fail.
- **A content hash per object, because Pulumi compares inputs.** The input for a file upload is a *path*, which does not change when the bytes behind it do — so without `sourceHash` a redeploy reports no changes and keeps serving the previous build, which is indistinguishable from a deploy that worked. It matters for exactly one file: hashed assets get new keys and are therefore new objects anyway, and `index.html` is the one whose name never changes and which names all the others.
- **A missing or half-written build is a synth failure.** `collectAssets` throws — naming the directory it looked in — when the tree is absent, empty, or has no `index.html`. `vite build` into a fresh tree that then fails leaves exactly the third case, and uploading whatever was there would take the console down while every resource in the stack reported success.

**Two departures from the text.** The row asks for the API URL to be injected at build time; P0-17a put the SPA and the API on one distribution, so `/v1/...` is same-origin and `session.ts` addresses it relatively — injecting an absolute URL would create a second source of truth whose disagreement shows up as a cross-origin request dropping the session cookie, a login that appears to succeed and then behaves as though it did not. And the row asks for **cache invalidation**: there is none, deliberately. The default behaviour uses the legacy TTL fields with `minTtl: 0`, so CloudFront honours the origin's `Cache-Control`, and `index.html` is uploaded `no-cache` — the edge revalidates against S3 and picks up a new shell immediately, while hashed assets never need invalidating because their keys change. That reasoning depends on the cache policy on that behaviour, which is why it is written down beside it rather than left as silence.

**⚠ The row's own test is a post-deploy smoke check and has not run.** Nothing in this repository has been deployed since the `dev` stage was removed, so "root returns 200 with the expected `cache-control`, and a deep link returns the shell rather than a 404" is unverified against real CloudFront. What *is* verified: the header decision, the key normalisation, the failure modes, and — by CI grep, verified by deleting the import — that `sst.config.ts` actually pulls the module in. **The deploy order is now load-bearing: `pnpm --filter @catalogorosso/dashboard build` before `sst deploy`,** or the deploy fails at synth rather than shipping a stale console.
...
- **Review fix (2026-09-15): a network failure retries, and a retry cannot send twice.**
  - **The bug.** A `fetch` that rejected — DNS, a reset connection — escaped `resendTransport` as a bare `TypeError`, which `sendEmail` reads as not retryable. A connection blip abandoned a password reset on its first attempt, and a connection that hung waited for the API function's own timeout.
...
**A third, found by CI, and the fix for it was wrong twice before it was right.** The duplicate-SKU branch never ran against real Postgres.

The first diagnosis was that `pgErrorCode` read the SQLSTATE only from `error.cause.code` — where `db.execute` leaves it — while the query builder passes postgres-js's `PostgresError` through with `code` on the error itself. True, and not the actual problem: reading both levels changed nothing, because **the error never reaches a `catch` that can do anything useful**. postgres-js marks a transaction failed the instant any statement errors, rolls it back, and rejects the *outer* `transaction()` promise with the original error whatever the callback returned. Postgres agrees — after an error the session refuses every further statement until the transaction ends — so there is nothing to continue into even if the driver allowed it.

**Catching a constraint violation inside a transaction and carrying on is not a thing that works**, and it is the sort of belief a unit test with a fake transaction will confirm forever. The fix is to never raise: `ON CONFLICT (tenant_id, sku) DO NOTHING ... RETURNING` makes the conflict a *result* — an empty row set — with no exception, no poisoned transaction, one round trip, and no race between checking and inserting. The target is named rather than bare, so a violation of any other constraint still raises; a bare `DO NOTHING` would swallow the next unique index somebody adds and answer 201 for a product that was never stored. The update path cannot use this — `UPDATE` has no way to distinguish "conflicted" from "matched nothing" — so **P1-03 uses a savepoint instead**, which is the other way to survive an error inside a transaction.

**Two defects found by building this, both fixed here.**

- **`productInsert`'s inferred type disagreed with its runtime schema (P0-42).** Refining `stockQty` — a nullable column — made the *type* demand a value while the schema went on accepting an omission. So `parse()` returned an object whose `stockQty` was `undefined` under a type saying it could not be, and any caller doing arithmetic on it was reading something the compiler had endorsed and the data did not support. `.nullish()` makes the two agree; the runtime behaviour is byte-identical, verified against omitted, null, valid, negative, fractional and string inputs. `contracts.test.ts` now pins it with an assignment that fails `pnpm typecheck` if it drifts back — and the fix made a cast in `packages/testing/src/factories.ts` redundant, which is where the wrong type had been quietly worked around.
- **The route returned the stored row verbatim, leaking `content_hash` and `tenant_id`.** Neither is in the published contract. The hash is an internal cost control, and a client that could see it would eventually branch on it — turning a change in how it is computed into a breaking API change instead of a re-index. `toProductResponse` projects explicitly, so a column added to the table is absent from responses until somebody decides otherwise, and the test asserts the body against a **strict** parse so the guarantee holds for the next column rather than these two. Found by a test rather than a reviewer.
...
**The savepoint, and why the create path does not need one.** A constraint violation inside a transaction cannot simply be caught: postgres-js marks the transaction failed on any statement error and rejects the *outer* `transaction()` promise with it, whatever the `catch` returned (see P1-02's third defect). `insertProduct` sidesteps that with `ON CONFLICT DO NOTHING`, which never raises. `UPDATE` has no equivalent — a `DO NOTHING` there could not be told apart from "matched nothing" and would report a silent success for a SKU that was never changed — so the statement runs inside a nested transaction, which Drizzle issues as `SAVEPOINT`. The violation then rolls back to the savepoint rather than poisoning the whole transaction, and only then is there something a `catch` can act on. The unit test counts the savepoint, so the design is asserted rather than assumed.
...
- **The cursor value is cast in SQL, not in JavaScript, and two failures got here first.** Handing the cursor's string straight to a `timestamp` comparison throws inside Drizzle's own driver mapping; converting it back to a `Date` fixes that and breaks paging, because a JS `Date` holds **milliseconds** and a Postgres `timestamptz` holds **microseconds** — so the cursor names an instant slightly *before* the row it came from. Descending, paging stopped after two pages; ascending, it never terminated. The value is now selected from Postgres with a `::text` cast and compared back with an explicit `::timestamptz`, which is exact and still lets the comparison use the index. Both failures were found by the integration suite, and neither is visible to a fake: it neither maps driver values nor stores a timestamp.
...
- **A `grape` filter was added, and it is not a convenience.** P1-07 could not fold the grape array into the searchable column — `array_to_string` is `STABLE` — so free-text search cannot answer "find me a nebbiolo" at all, and this filter is the only place that question can go. Containment against the array GIN index, which is what makes it affordable.
...
- **An abandoned claim expires after 30 seconds, three times the API function's timeout** *(addition; review fix 2026-09-15)*. It was 15 minutes, the longest *any* Lambda can run, which refused a seller's retry of a cut-off import for a quarter of an hour on a function killed at 10 seconds. A claim with no result older than that belongs to an invocation that no longer exists, and is taken over — for the same body only. There is no release-on-error: nothing that returns leaves a claim open, and a release would run on the connection that just failed to store the result.
...
- **The key's header name and the claim-expiry constant live in the API and `packages/db`.** P1-23 is the first client; it moves the header name somewhere the dashboard can import rather than retyping it.

Tests: `packages/db/test/import-runs.test.ts` and `schema/import-runs.test.ts` (unit), `import-runs.integration.test.ts` (replay, both 409s, the race, takeover one second past the expiry and not one second before it, a completed run never taken over, two wineries sharing a key); `apps/api/test/products-import.test.ts` (the route's P1-26 block), `products-import-port.test.ts`, and `products-port.integration.test.ts` (replay and cross-tenant keys through the real port).
...
**`updated_at` moved on a re-index, and migration `0038` is why it no longer does.** The status write does not set the column; P0-22's `BEFORE UPDATE` trigger did, unconditionally, on every table — so a bulk re-index moved every wine to the top of "recently edited" without a seller having touched one. That is not an internal timestamp: P1-06 offers `updatedAt` as a sort field, so it is a thing sellers order their catalogue by, and a sort that reshuffles itself for invisible reasons is worse than no sort because they believe it.

`products` now carries its own trigger that ignores `embedding_state`, `embedding_error` and `embedding_attempts`. **A narrower trigger rather than a second `edited_at` column**: two timestamps would mean every future reader choosing between them, and the one thing the second would have recorded — when a wine's embedding last changed — is already stored beside the vector in `product_embeddings.created_at`. The comparison excludes those three columns rather than listing the ones that count, so a column added later is an edit *by default*: forgetting to update the function makes a stamp move slightly too often, never too rarely, and too rarely is the failure a seller cannot see.

**The first version of this trigger was a no-op, and nothing about it looked wrong.** Postgres computes a stored generated column *after* BEFORE triggers run, so inside the function `NEW.search_tsv` is NULL while `OLD.search_tsv` holds the last write's value (P1-07 added that column). The two rows were therefore *always* distinct, whatever the statement changed — so the narrowing did nothing and every embedding write still moved the stamp. A minimal reproduction confirmed the logic on a table with no generated column, which is exactly the wrong thing to reproduce. The list now excludes it, and `products_generated_columns_are_ignored` reads both lists out of the catalogue so the next generated column cannot break it the same invisible way.
...
**The tooltip explains the state, not the provider's reason** *(deviation)*. `embedding_error` is not in the product response, and that is a recorded decision rather than an oversight: `product-contracts.test.ts` lists it as withheld because it holds an error *name* — "ValidationException" tells a winery nothing — and publishing it before P1-50 replaces it with a seller-readable sentence would set a contract around a string that is meant to change. So each state's tooltip says what it means *for the seller* ("non viene consigliato… usa «Reindicizza» per riprovare"), which is true now and stays true once P1-50's reason sits beside it. "The error tooltip shows the reason" becomes "the failed tooltip names the consequence and the action". **Since P1-50 it adds the reason too**, as the sentence for the published `embeddingFailure` code, after the state's own.
...
**As built — the row's three classes, with one rule added that decides whether "permanent" is safe at all.**

- **The classifier lives in `packages/core`** (`rag/embedding-failure.ts`) *(deviation — the row named `apps/worker/src/error-classify.ts`)*. The reason set is the contract between the worker that writes it and the API that publishes it, and the API cannot import the worker. It is pure and name-based, and under core's 90% gate.
- **Reasons are stored as codes** in `embedding_error`: `input-rejected`, `service-unavailable`, `unknown` *(decision — the row quotes an Italian sentence)*. The console words them, so the wording can change without a contract change or a data migration. The API publishes them as a derived `embeddingFailure`; the raw column stays withheld, because wines failed before P1-50 store provider error names, which read as `unknown`.
- **Only a failure of the provider call can be permanent or unknown** *(addition)*. The worker wraps that call in `EmbeddingProviderError`; anything else — a database connection dropped mid-run, a bug of ours — is transient. Without the wrapper, the row's "unknown: transient once, then permanent" would turn a database outage into every in-flight wine permanently failed and acknowledged, with the DLQ alarm never firing.
- **Transient**: the adapter's retry names (now one set in core, which the Titan adapter reads), plus **operator errors** — credentials, permissions, a model not enabled, a quota *(addition)*. Those are ours to fix, so they go behind the DLQ alarm rather than reading as a seller's problem. Recorded as `service-unavailable` and rethrown.
- **Permanent**: `ValidationException` becomes `input-rejected`, recorded and acknowledged, with zero SQS redeliveries and never the DLQ. The adapter already did not retry it in-call, so it costs one provider call in total.
- **Unknown**: rethrown on the first delivery, then recorded as `unknown` and acknowledged from the second, counted with SQS's `ApproximateReceiveCount`. A missing or unreadable count is a first delivery.
- **"Input too long" and "empty after sanitisation" are not reachable as the row describes them.** P1-36 truncates an over-long text at a sentence boundary before sending, so it is embedded shortened rather than refused, and every wine has a required name, so the text is never empty. A refusal that still happens arrives as `ValidationException`. **Telling a seller their note was shortened is open**: it is a warning on an indexed wine, not a failure, and nothing carries it yet.
- **The grid's tooltip adds the reason's sentence after the state's.** The row's manual **Riprova** is P1-40's existing «Reindicizza», which the transient and unknown sentences point to. The `input-rejected` sentence points at editing instead, because saving re-queues the wine by itself (P1-38's `edited` edge, FAILED → PENDING) and reindexing refused text only fails again.
- **The redrive is `redriveDlq` in `apps/worker`, with `scripts/redrive-dlq.mjs` as its entry point** *(deviation — `.mjs`, like every other script)*. It uses SQS's own `StartMessageMoveTask` rather than a receive-send-delete loop, which can neither duplicate nor lose a message between the two queues. It refuses to start while a move is running, so running it twice is safe, and redelivered messages are idempotent at the worker. A person runs it with their own credentials; nothing in CI or the deployed system does.
- `docs/runbooks/embedding-failures.md` is the alarm's response: what each reason means, how to triage from the worker's log lines, and how to redrive.
...
**As built — a peer adapter in `packages/llm`, and the provider suite written once.**

- **`responseMimeType: 'application/json'` with a `responseSchema` projected from `pairingJsonSchema()`** by `toGeminiSchema`, an allowlist: object, array, string and number with their bounds, plus `propertyOrdering` so the reply comes first. **`additionalProperties`, `format` and `pattern` are dropped** *(decision)* — Gemini's schema subset rejects keywords it does not know, and none is needed to produce the shape; `parsePairingOutput` still strips unknown keys and refuses a non-UUID id, which counts toward the schema-failure rate. A type the projection cannot express throws, so a schema change fails a unit test rather than every request. `responseSchema` rather than the newer `responseJsonSchema`, as the row says: its SDK type is the documented subset, so `tsc` checks the projection.
- **The JSON is never streamed as text** *(decision)*. Gemini's whole answer is JSON text, so a delta is a fragment of JSON, not a sentence. It is accumulated, checked by the same `trustedPairing` as Nova's tool input, and the reply is said once it has passed. **Open for P1-47:** this costs time to first token, which the bake-off table has to show; extracting the reply from the partial JSON as it streams is the fix if it matters.
- **Refusals** are a blocked prompt (`promptFeedback.blockReason`) or a finish reason of `SAFETY`, `RECITATION`, `LANGUAGE`, `BLOCKLIST`, `PROHIBITED_CONTENT`, `SPII`, `IMAGE_SAFETY` or `IMAGE_PROHIBITED_CONTENT`. `MAX_TOKENS` is not a refusal: the truncated JSON fails validation and is `schema_invalid`. Thought-summary parts are never read as the answer.
- **The key is passed in, never read from the environment** *(deviation — the row says "API key from SSM", which is the caller's job)*. `geminiProvider` takes an `apiKey` or a `client` and throws given neither, or an empty key, because the SDK would otherwise fall back to `GEMINI_API_KEY` — a second place for a key that lives in SSM. Reading SSM belongs to the chat route (P2-29) and the bake-off command (P1-47).
- **Usage reports cache reads apart from billed input**, as Bedrock does: Gemini counts `cachedContentTokenCount` inside `promptTokenCount`, so it is subtracted, and thinking tokens are added to output because they are billed as output. Caching is Gemini's implicit prefix cache over the byte-identical system instruction; nothing asserts a hit, because implicit caching has a minimum prefix and no guarantee. `gemini.live.test.ts`, **skipped unless `LIVE_GEMINI=1`**, proves instead that Gemini accepts the projected schema and answers with a pairing that passes validation.
- **Abort is handed to the SDK as `config.abortSignal`** and checked between events. The SDK documents it as client-side only — the connection closes, but the service is not promised to stop generating — so P1-47's cost figures cannot assume an abandoned answer is free.
- **The provider suite is written once**, `test/provider-contract.ts`, and both adapters' test files run it with fakes in their own wire formats: a valid answer, an empty reply, five kinds of untrustworthy output, a refusal, a rejected request, a broken stream, abort in flight, mid-stream and before sending, and usage. Nova's file keeps only what is Nova's: text streamed beside the tool call, its stop reasons and exception events, and the request.
- **Two pieces of Nova's adapter moved out to be shared** *(refactor)*: `alternatingTurns` in `src/turns.ts` and `trustedPairing` in `src/pairing.ts`, so "checked before it is believed" is one function whichever model answered.
- **`@google/genai` is answered `false` in `allowBuilds`**: its install scripts are `echo` no-ops.
...
**As built — the harness in `packages/eval`, with retrieval and the judge as seams until the rows that fill them exist.**

- **In `packages/eval`, beside the dataset** *(deviation — the row names `packages/testing/src/eval/harness.ts`)*, for P1-45's reason: the harness needs `@catalogorosso/core` and `@catalogorosso/llm`, and `packages/testing` cannot depend on core.
- **Retrieval is a `Retriever` seam, not real retrieval** *(deviation — P2-18 to P2-20 are not built)*. The row seeds a Testcontainers database, indexes it with the real embedding provider and runs real retrieval, and there is no retrieval function yet to run. The harness seeds the catalogues in memory with fresh ids and scores whatever ranking it is handed. CI and `pnpm eval` use `lexicalRetriever`, a deterministic keyword match over each wine's embedding text that returns only wines sharing a word with the query — **a stand-in, and its recall is not a finding**. **Open, owned by P2-20:** the database-seeded, embedding-indexed path plugs in at the seam, and recall@8 and MRR mean something from then.
- **The scores.** Recall@8 and MRR over answerable queries; the schema-failure, refusal and provider-error rates over every call; the pairing-hit rate — an acceptable wine among the recommendations; an **honesty rate** *(addition)* — unanswerable queries answered without recommending anything, rather than failing or confabulating; and a count of **recommendations naming a product that was not a candidate** *(addition — each one a P2-25 drop)*. A recommended id scores only through the candidates the model was handed, never by looking it up in the catalogue.
- **Pairing quality is a `Judge` port, with the rubric fixed here** *(deviation — no judge model is wired)*. `PAIRING_RUBRIC` scores the reason against the five mechanics above and marks a defensible wine with a wrong reason `rejected`; the judge is shown exactly the candidates the model saw, so attribute grounding is checkable. Which stronger model applies it needs credentials and spend, so it is chosen and wired in **P1-47**. `humanSample` reserves the human-rated sample by a hash of the query id, so it never moves between runs or raters and grows without reshuffling.
- **The markdown table** has a row per provider with both runs in every cell, a **spread** column — the widest difference between the two runs across the rates, in points — so a difference inside run-to-run noise is visibly not a finding, and a mechanical verdict: a schema-failure rate above 2% in either run is *disqualified*.
- **One query at a time**, so the provider-error rate measures the provider rather than our own burst, and a run's order is the dataset's.
- **The deterministic CI run is `harness.test.ts`**: the whole dataset through the lexical retriever and `fakeLlmProvider`, twice, asserting identical results. **The real run is `pnpm eval`**, opt-in by construction: no test imports `src/cli.ts`, and `providerFromEnv` refuses to start without `EVAL_PROVIDER`, `EVAL_MODEL` and the provider's key from the shell that runs it, so a misconfigured run stops before it spends anything. It runs twice and prints the table and the human sample.
- **The reusable fake provider landed here**, `src/fake-llm-provider.ts`, as P1-41 recorded it would.
...
- **Gemini's projection is narrower than the JSON Schema** *(P1-43)*. `toGeminiSchema` drops `additionalProperties`, `format` and `pattern`, which Gemini's `responseSchema` subset does not accept; the Zod schema stays the single source, and `parsePairingOutput` enforces what the projection cannot.
...
**The escalation-rate alarm, closed (2026-09-22).** The row asks for one when the rate exceeds a few percent, and a *rate* needs a denominator — turns — which did not exist until P2-29 streamed them and P2-31 counted them; an alarm on an absolute count would have fired on a busy Saturday, which is the opposite of what the row wants. Both numbers now come out of one EMF line the chat route writes, and `ChatEscalationRate` divides them. The expression returns *nothing* under twenty turns an hour, because a rate over two turns is a statistic about nothing, and `notBreaching` is what turns that silence into quiet rather than into an alarm people learn to ignore.
...
**As built.** `randomBytes(32)` hex and the constant-time compare landed with P4-01 and P4-02, where they were needed; this row is the lifetime.

- **The window is computed in SQL, `now() + interval '7 days'`** *(addition)*. A clock the caller supplies is a clock the caller can move — and across a fleet of Lambdas, "the caller's clock" is several clocks that disagree.
- **A lapsed nonce is replaced, never extended** *(addition; "issues a fresh nonce" said what, not why)*. It has been sitting in a public TXT record for a week, so anybody who looked has a copy. Extending the window would mean the thing that proves control of a domain is a thing a passer-by can replay.
- **The check runs before the lookup, not after it.** Checking a lapsed nonce against the record and reissuing afterwards would accept an expired proof for as long as it takes somebody to notice.
- **The seller is told the value changed, not that verification failed.** To them the record they published is still sitting there, and "it does not match" with no explanation is the version of this that generates a support ticket.
- **A row with no deadline is checked, not treated as lapsed.** Rows created before this row existed carry none, and reissuing on sight would hand a fresh nonce to every seller who had just published the old one.
- **Reissuing still spends the rate-limit bucket** (P4-02). It is a write and a round trip; an expired nonce is not a free pass through the thing that bounds this endpoint.
- **`verificationExpiresAt` is on the wire**, because the screen has to say it. A seller who publishes a record and comes back a fortnight later needs to be told the value changed rather than left wondering why a correct-looking record will not verify.

**A shipping defect this row found, in P4-01's code.** A raw `tx.execute` returns `timestamptz` columns as **strings**, not `Date` objects — Drizzle parses column types for a typed select and does nothing of the kind for a hand-written statement. The row type said `Date`, TypeScript believed it, and `createdAt.toISOString()` in the domains port would have thrown on **every domain add**. No unit test could see it, because a mocked driver returns whatever the test says; the integration suite found it on the first run that read a timestamp back. `toDomain` now coerces through an `asDate` helper with the reasoning written beside it, and there is a unit case pinning the string form. **The same pattern was untouched in `invitations.ts`, `with-invitation.ts` and `memberships.ts`** — swept in the follow-up below.

**The sweep (Review).** `packages/db/src/timestamps.ts` now holds the one coercion, and the three remaining sites use it: `readOpenInvitations`, `readRoster` and `withInvitation`.

The members screen was the worse case, and quieter than the domains port's. It does not throw a `TypeError` — it ships `2026-10-02 01:01:04.326752+00` to a dashboard whose client parses every response against `z.iso.datetime()`. A space instead of a `T` and `+00` instead of `+00:00` are exactly what an ISO parser rejects, so `.parse()` throws on the client and **the roster and pending-invitation screens do not load at all**.

`rate-limit.ts` had already found this once and written down why it hides: the test double and the code were written from the same assumption, so they agreed with each other and not with Postgres. The reason is now in one place with the helper, rather than in a comment on the one module that got burned. `products-read.ts` keeps its cursor as a string deliberately and is left alone — a `Date` holds milliseconds and a `timestamptz` holds microseconds, which is its own note.

**Verified.** 10 more unit cases on the statements, 9 on the port, 5 more against real Postgres — which is where the seven-day arithmetic is actually exercised, since the database computes it. A mutation run of 19 mutants killed 19, after two survivors were fixed: nothing asserted that a reissue which changed nothing writes no audit row, and nothing asserted the new deadline reaches the wire.
...
- **+ (review, 2026-09-14)** **Those fixture tests now exist**; the bullet above promised them and nothing wrote them, so every refusal the gate makes had been proven by hand, once, when this row landed, and never again. `scripts/test/check-coverage.test.mjs` runs the script as CI does — a child process, judged by exit status — against summaries built from the packages on disk: all bars met, a bar missed, `packages/security` at 99.99%, the vacuous pass, a package with no files, and no summary at all. The gate scripts are `.mjs` outside every tsconfig, so this needed a `scripts` project in `vitest.config.ts` with an include of its own. Mutation-checked: deleting the vacuous-pass check, dropping the `failed++`, or rounding in a package's favour each turns it red.
...
| 🔒 Budget alerts are silent until confirmed | **action required — [runbook](docs/runbooks/budget-alerts.md)** | The secret is set and both budgets exist on `dev`, but the SNS subscription is `PendingConfirmation` — AWS sends a confirmation link to the address and delivers nothing until it is clicked. So the cost control that makes an always-on NAT and RDS acceptable is currently not connected, and the failure is silent by nature: no alert looks identical to no spend. Confirm the subscription for `grossomatteo.aws@gmail.com`, and re-check `aws sns list-subscriptions` after creating any new stage. |
...
**The mechanism, measured.** Four probes against the deployed stage, read back as `xffEntries` on the request line:

| Probe | `xffEntries` |
|---|---|
| Through CloudFront, forging `X-Forwarded-For: 1.2.3.4` | 1 |
| Through CloudFront, no header | 1 |
| Through CloudFront, forging three entries | 1 |
| Direct to the Function URL, forging one | 1 ← the bypass |

The first three closed the original question: the CloudFront Function overwrites with `event.viewer.ip`, a forgery of any length collapses to one entry, and — the specific worry that motivated measuring — **CloudFront does not append its own entry after a viewer-request function runs**. Had it done so, every request would have arrived as two entries, `clientIp` would have declined all of them, and the limiter would have bucketed every caller together while the configuration read correctly.

**Review fix (2026-09-15): `clientIp` parses an address rather than matching its alphabet.** One entry was necessary and not sufficient: the check admitted anything made of hex digits, dots and colons — `cafe`, `1.2.3`, `999.1.1.1`, `:::` — so a value that is not an address still reached the request context, where `audit_log.ip` (`inet`) refuses it mid-write, and the widget limiter, where it names a bucket. It is now `node:net`'s `isIP`. Behind the edge this cannot happen, since the viewer function writes a real address; the Function URL was reachable directly until the origin secret shipped, and the parser should not depend on the edge being in front of it.

**The fourth was the finding, and closing it took two attempts.**

*Origin Access Control, tried and reverted.* `url: { authorization: 'iam' }` with a `lambda`-type OAC locked the origin correctly for GET — 403 direct, 200 through the distribution — and broke **every POST**, including sign-in: `The request signature we calculated does not match`. CloudFront does not sign the request body for a Lambda origin, so an `AWS_IAM` Function URL rejects anything that has one. The remaining options were worse: `oac` requires the *client* to compute `x-amz-content-sha256`, impossible for a widget on a seller's site, and `oac-with-edge-signing` puts Lambda@Edge on every request — us-east-1 only, per-call latency, a 1MB body cap.

*A shared secret header, which is what shipped.* CloudFront attaches `x-origin-secret` at the **origin-request** stage, where a viewer cannot see it and a viewer-supplied header of the same name is replaced rather than merged. The API refuses anything without it — 404, not 403, on §3.5's reasoning: 403 would confirm to a prober that they had found the origin and needed only the header.

Verified on the live stage, both directions:

| | Through CloudFront | Direct to the Function URL |
|---|---|---|
| `GET /v1/health` | 200 | **404** |
| `GET` with a forged `X-Forwarded-For` | 200 | **404** |
| `POST` sign-in | 200 | **404** |
| A guessed header | — | **404** |

**What this gives up, stated rather than glossed.** A shared secret is a bearer token, not a signature: anyone who can read the distribution's configuration can replay it. It defends against the internet, not against an insider with console access — and that is the larger and far likelier half. Rotation is deleting the resource and redeploying, which is **not zero-downtime**, since the origin and the function pick up the new value at slightly different moments.

**Three things keep it from decaying.** The secret is generated by `RandomPassword` rather than set by an operator, so there is no step to forget. `apps/api/src/index.ts` **refuses to start a deployed stage without it** — absent is permissive here, which is the one shape E9 established must never be a silent default. And `ci.yml` asserts both halves are wired, because dropping either leaves an API that works perfectly and is quietly reachable around the edge, which is exactly how this went unnoticed the first time.

**The function bodies are executed in CI now (review, 2026-09-14).** What was measured above is what CloudFront did with the deployed code on one day; nothing re-ran that code afterwards, because it was a string literal inside `infra/cdn.ts`, and that module constructs SST resources at import and cannot be loaded outside a deploy. Both bodies moved into `infra/edge-functions.ts` — the `queue-config.ts` split, for the same reason — and `infra/test/edge-functions.test.ts` evaluates those exact strings as the runtime does: a forged `X-Forwarded-For` is replaced rather than kept, any list collapses to one entry, other headers pass untouched, extensionless routes rewrite to `/index.html` and assets do not, and an `/invito/<token>` path rewrites for tokens drawn from the base64url alphabet that has no dot in it (**E10**). `typecheck:infra-pure` found the new module by itself, which is the point of that gate deriving its file list. Mutation-checked: an appending `ViewerIp` and a rewrite that sends `/favicon.ico` to the shell each fail it. The same review also found that no test asserted `index.ts` refuses a deployed stage without `ORIGIN_SECRET`, or that `buildDependencies` hands the secret on — deleting either line left the whole suite green, which is this item's failure arriving from inside the API. `test/index.test.ts` now loads the entry point cold per case and sends Function URL events through the real handler; `test/composition.test.ts` asserts both secrets through requests.

**A2b. Two things the deploy confirmed by inference rather than by a dedicated test.** ✅

**NAT egress works.** The account has *no* VPC endpoints, so the migration runner's successful `GetParameters` call could only have left the private subnet through NAT. That matters beyond SSM: it is the same path Resend, Stripe and Bedrock take, and none of them had been exercised. A broken NAT would have surfaced as three unrelated features failing later.

**The origin guard did not disturb the edge.** Eight requests after the change, every one logged `xffEntries: 1` — the header is still being overwritten by the CloudFront Function and still resolves to a single usable entry. Worth checking rather than assuming, because the change touched the origin's configuration and a regression there would have been silent: the API keeps working, and only the rate limiter quietly stops distinguishing callers.
...
**E10. An invitation link lands on a page that does not exist.** ⚠ **open (2026-09-14)**

Found by the whole-codebase review. The invite email carries `<AUTH_BASE_URL>/invito/<token>` (`acceptUrlBase` in `composition.ts`), and the console's router has `/`, `/catalogo`, `/membri` and a catch-all. So a signed-in invitee who already belongs to another winery sees *Pagina non trovata*, one with no winery yet sees an empty *Scegli una cantina*, and a signed-out one is sent to sign in — and in all three **nothing ever calls `POST /v1/dashboard/members/accept`**. The owner is told "invited"; every invitation expires unused.

Everything behind the screen exists and is tested: the endpoint, mounted above `resolveTenant` so a non-member can reach it; `withInvitation`'s single-use lock against real Postgres; and, since this review, `SpaRewrite` serving the console for `/invito/<token>` across the base64url alphabet the token is drawn from. What is missing is one screen: read the token from the path, get the invitee signed in or signed up *with the invited address* — acceptance refuses any other — redeem it, and land them in the winery it names.

No row tracks it. **E8** closed the members API and leaves `/membri` a placeholder; P0-57 built the shell and deferred its screens. Neither mentions acceptance, which is the one flow where the person in the console is not a member of anything yet — so it cannot be a tab inside `Layout`, which assumes a membership to render around, and needs a route of its own beside `RequireAuth`.
