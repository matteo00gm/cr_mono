...
Three assertions, because a trigger that never fires is not a narrower trigger: a status write leaves the stamp, an ordinary edit moves it, and a column nobody thought about moves it. (The first version of this code claimed it opted out of the shared trigger; CI's integration run is what said otherwise.)
