...
- **Never let a vendor SDK read its credentials from the environment.** Keys come from SSM through the caller and are passed in; a factory given none throws rather than let the SDK fall back to `GEMINI_API_KEY` (P1-43).
